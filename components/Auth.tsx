...
export const Auth: React.FC<AuthProps> = ({
    ...
    try {
      if (isLogin) {
        ...
        if (data.user && !data.session) {
          ...
          alert("Account created! Please check your email for a verification link, then sign in.");
          setIsLogin(true);
        } else {
  ...
  const handleLiveDemo = async () => {
    // Directly use offline demo mode
    if (onRunOfflineDemo) {
      onRunOfflineDemo();
    }
  };
    ...
    <div className="min-h-screen bg-white relative font-sans text-slate-900 selection:bg-blue-100 overflow-x-hidden">
      ...
      <Modal isOpen={showAuthModal} onClose={handleCloseModal} hideHeader={true}>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {/* Google Logo */}
